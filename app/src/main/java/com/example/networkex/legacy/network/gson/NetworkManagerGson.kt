package com.example.networkex.legacy.network.gson

import com.example.networkex.legacy.const.ApiConst.SELF04_CODE
import com.example.networkex.legacy.const.ApiConst.SELF04_HEADER_MSG_TYPE
import com.example.networkex.legacy.network.NetworkInterceptor.commonInterceptor
import com.example.networkex.legacy.network.NetworkInterceptor.konaCardInterceptor
import com.example.networkex.legacy.network.NetworkInterceptor.misAuthInterceptor
import com.example.networkex.legacy.network.NetworkProvider.provideRetrofit
import com.example.networkex.legacy.network.model.gson.KonaCardRequestBodyUserId
import com.example.networkex.legacy.network.model.gson.SelfRequestBody
import com.example.networkex.legacy.network.model.gson.SelfRequestInnerBody
import com.example.networkex.legacy.network.model.gson.SelfRequestInnerHeader
import com.example.networkex.legacy.util.NetworkUtil
...
class NetworkManagerGson {
    ...
    fun requestMembershipPoint(membershipId: String, authToken: String): Response<Any> {
        val networkService: NetworkServiceGson = provideRetrofit(konaCardInterceptor(authToken)).create(
            NetworkServiceGson::class.java)
        return networkService.pointInfo(KonaCardRequestBodyUserId(membershipId)).execute()
    }
